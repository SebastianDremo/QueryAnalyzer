...
options { tokenVocab=SqlLexer; caseInsensitive=true; }
...
alter_table
    : ALTER TABLE table_name (SET '(' LOCK_ESCALATION '=' (AUTO | TABLE | DISABLE) ')'
                             ...
                             | WITH (CHECK | NOCHECK) ADD (CONSTRAINT constraint=id_)?
                                ( FOREIGN KEY '(' fk=column_name_list ')' REFERENCES fk_table_name=table_name ('(' pk=column_name_list')')? (on_delete | on_update)*
...
xml_type_definition
    :  (XML | SQUARE_BRACKET_XML) '(' ( CONTENT | DOCUMENT )? xml_schema_collection ')'
    ;

xml_schema_collection
    : (ID | SQUARE_BRACKET_ID) '.' (ID | SQUARE_BRACKET_ID)
    ;
...
column_definition
    : id_ (data_type | AS expression PERSISTED? )
        (generated_column_constraint | dynamic_masked_column)?
         (COLLATE id_)? null_notnull?
...
table_constraint
    : (CONSTRAINT constraint=id_)?
       ((PRIMARY KEY | UNIQUE) clustered? '(' column_name_list_with_order ')' index_options? (ON id_)?
         | CHECK (NOT FOR REPLICATION)? '(' search_condition ')'
         | DEFAULT '('?'('?  table_constraint_default_value ')'?')'? FOR id_
         | FOREIGN KEY '(' fk = column_name_list ')' REFERENCES fk_table=table_name ('(' pk = column_name_list')')? on_delete? on_update?)
    ;
    
table_constraint_default_value
    : ((STRING | PLUS | function_call | DECIMAL | FLOAT | REAL)+ | NEXT VALUE FOR table_name)
    ;
...
generated_column_constraint:
    GENERATED (ALWAYS | NEVER) AS (ROW | TABLE | FUNCTION) (START | END)
    ;
    
dynamic_masked_column: 
    MASKED WITH '('FUNCTION '=' masked_with_valuje=id_or_string ')'
    ;
